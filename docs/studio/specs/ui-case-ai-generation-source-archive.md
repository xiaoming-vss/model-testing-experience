...
源码包是 UI 生成任务唯一的生成来源。前端不展示或提交 `sourceContent`，不读取或解压源码包内容，也不调用任何内部 Worker 接口。审核完成后候选结果只读；本规格原始交付止于审核，后续导入能力见 `docs/studio/specs/ui-approved-candidate-import.md`。
...
- 本规格交付时不调用 UI 套件导入端点；后续实现以 `docs/studio/specs/ui-approved-candidate-import.md` 为准。
